...
const Bs = (props: {ship: Ship; modShip: (ship: Ship) => void}) => {
				...
				if (newShield === "personal") {
					props.modShip(
						new Ship(
							...
							"bs",
							props.ship.hullHp,
							props.ship.personalShieldHp,
							0,
							0,
							0,
							props.ship.coordinate,
						),
					);
				}
				if (newShield === "area") {
	...
	const handleShieldLevelChange = useCallback(
		(event: SelectChangeEvent) => {
	...
	const handleShieldHpChange = useCallback(
		...
		(event: SelectChangeEvent) => {
			...
			props.modShip(
				new Ship(
					props.ship.id,
					"bs",
					fullHpByBsLevel(level),
					props.ship.personalShieldHp,
					props.ship.blastShieldHp,
					props.ship.areaShieldLevel,
					props.ship.areaShieldHp,
					props.ship.coordinate,
				),
			);
		},
	...
	return (
		...
		>
			...
			/>
			{props.ship.chanceOfDeath !== null && (
				<Typography variant="body1">
					Chance of death: {Math.round(props.ship.chanceOfDeath * 10000) / 100}%
				</Typography>
			)}
			<FormControl>
				<TextField
			...
			<FormControl>
				...
				<Select value={shield} label="Shield" onChange={handleShieldChange}>
