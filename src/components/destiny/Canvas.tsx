...
const Canvas = () => {
	...
	const [mouseCoords, setMouseCoords] = useState<Coordinate>(
		new Coordinate(0, 0),
	);
	...
	const addShip = useCallback(
		(s: Ship) => {
			setShips((prevState) => {
				return [...prevState, s].sort((a, b) =>
					b.type !== a.type && (a.type === "bs" || b.type === "bs")
						? a.type === "bs"
							? -1
							: 1
						: 0,
	...
	const attackerMod = useCallback(
		(oldS: Attacker) => (newS: Attacker) => {
	...
	const removeAttacker = useCallback(
		(oldS: Attacker) => () => {
			setAttackers((prevState) => {
				return prevState.filter((actualS) => actualS !== oldS);
			});
	...
	const solve = useCallback(() => {
		...
		setShips((prevState) => {
			return prevState.map((actualS) => {
				return actualS.setChanceOfDeath(
					ret.find((r) => r.id === actualS.id)?.dc || 0.0,
				);
			});
		});
		...
		<>
			...
			<Button
				...
				onClick={() =>
					setAttackers((prev) => [
						...prev,
						new Attacker(4, new Coordinate(2500, 2500)),
					])
				}
			>
				Add Attacker
			...
			<Button
				variant="contained"
				onClick={() =>
					addShip(createTransport("t1", new Coordinate(2500, 2500)))
				}
			>
				Add TS
			</Button>
			<Button
				variant="contained"
				onClick={() => addShip(createMiner("m1", new Coordinate(2500, 2500)))}
			>
				Add Miner
			</Button>
			<Button
				variant="contained"
				onClick={() =>
					addShip(createRelicDrone("rd1", new Coordinate(2500, 2500)))
				}
			>
				Add Relic drone
			</Button>
			...
			<Button variant="contained" color="warning" onClick={() => solve()}>
				Simulate
			</Button>
