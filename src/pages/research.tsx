...
const Row = (props: {
	module: ModuleArtifactInfo;
	bps: number;
	targetBps: number;
	setBps: (bps: number) => void;
	setTargetBps: (bps: number) => void;
	isSelected: boolean;
}) => {
	...
	const handleTargetBpsChange = useCallback(
		(event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
			const level = event.target.value as unknown as number;
	...
	const currLvl = useMemo(
		() =>
			`currentMaxLvl: ${
				props.module.milestones.findIndex((m) => m > props.bps) === -1
					? "max"
					: props.module.milestones.findIndex((m) => m > props.bps)
			}`,
		[props],
	);
	const targetLvl = useMemo(
		() =>
			`targetMaxLvl: ${
				props.module.milestones.findIndex((m) => m > props.targetBps) === -1
					? "max"
					: props.module.milestones.findIndex((m) => m > props.targetBps)
			}`,
		[props],
	);
...
const Home: NextPage = () => {
	...
	const mods = useMemo(() => {
		return getModuleInfo().sort((a, b) => a.TID.localeCompare(b.TID));
	}, []);
	...
	const setBpsToIdx = useCallback(
		(idx: number) => (newBps: number) => {
			...
			<Grid container spacing={2}>
				<Grid item>
					<ModuleIconSet
						mods={mods}
						selected={selected}
						setSelected={handleModuleChange}
					/>
				</Grid>
				<Grid
					item
				...
				>
					...
					>
						<FormControl fullWidth style={{marginTop: "12px"}}>
							...
							>
								{Array.from(
									Array(12 - Math.max((selected?.tier ?? 0) + 2, 4)).keys(),
								)
									.map((l) => l + Math.max((selected?.tier ?? 0) + 2, 4))
									.map((l) => (
						...
						<FormControl fullWidth style={{marginTop: "12px"}}>
							...
							<Select
								...
								fullWidth
							>
								{Array.from(Array(21).keys()).map((l) => (
					...
					{selected && (
						<>
							...
							<p>
								Selected mod siblings: {siblings.map((m) => m.name).join(", ")}
							</p>
							<p>
								{(() => {
									const drops = getDroprateToTier(selected.name, level)
										?.drop ?? [0, 0];
									return `Drops per artifact; ${drops[0]}(+${(
										(drops[0] * bonus) /
										100
									).toLocaleString("en-us", {maximumFractionDigits: 2})}) - ${
										drops[1]
									}(+${((drops[1] * bonus) / 100).toLocaleString("en-us", {
									...
									return (
										<>
											...
											<ul>
												<li>
													Avg;{" "}
													{`${out.avgCase} (${secondsToStr(
														out.avgCase * d.researchTime,
													)})`}
												</li>
												<li>
													Worst;{" "}
													{`${out.worstCase} (${secondsToStr(
														out.worstCase * d.researchTime,
													)})`}
												</li>
												<li>
													Best;{" "}
													{`${out.bestCase} (${secondsToStr(
														out.bestCase * d.researchTime,
													)})`}
												</li>
											</ul>
										</>
							...
							{!!siblings.length &&
								siblings.map((s, idx) => (
									<Row
										...
										bps={bps[idx] || 0}
										targetBps={targetBps}
										setBps={setBpsToIdx(idx)}
										setTargetBps={setTargetBps}
