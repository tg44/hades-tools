...
const Home: NextPage = () => {
	...
	const hUsagePerShipPer100Au = useMemo(
		() => getHUsage(tsLvl, cbeLvl),
		[tsLvl, cbeLvl],
	);
		...
		<Layout pages={pages}>
			...
			>
				<Card>
							...
							<Stack>
								<FormControl fullWidth style={{marginTop: "12px"}}>
									...
									<Select
										...
										fullWidth
									>
										{Array.from({length: 11}, (v, k) => k + 1).map((l) => (
								...
								<FormControlLabel
									control={
										<Checkbox
											defaultChecked={movementCalc}
											onChange={(e) => setMovementCalc(e.target.checked)}
										/>
									}
									label="Calc movement hydrogens"
								/>
								{movementCalc && (
									<FormControl fullWidth style={{marginTop: "12px"}}>
										...
										>
											{Array.from(
												{length: Ships.Transport.JumpFuelCosts.length},
												(v, k) => k + 1,
											).map((l) => (
												<MenuItem key={l} value={l}>
													{l}
									...
									<FormControl fullWidth style={{marginTop: "12px"}}>
										...
										>
											{Array.from(
												{
													length:
														Modules.TransportCapacity.UnlockBlueprints.length,
												},
												(v, k) => k + 1,
											).map((l) => (
												<MenuItem key={l} value={l}>
					...
					<CardActions>
						<Button size="small" color="primary" onClick={() => boxCb(1)}>
							{" "}
							Add Trade step{" "}
						</Button>
						<Button size="small" color="primary" onClick={() => boxCb(-1)}>
