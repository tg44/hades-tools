...
export class Ship {
	...
	copyWithCoordinates(x: number, y: number) {
		return new Ship(
			...
			this.type,
			this.hullHp,
			this.personalShieldHp,
			this.blastShieldHp,
			this.areaShieldLevel,
			this.areaShieldHp,
			new Coordinate(x, y),
		);
	}
	...
	constructor(
		...
		type: ShipType,
		hullHp: number,
		personalShieldHp: number = 0,
		blastShieldHp: number = 0,
		areaShieldLevel: number = 0,
		areaShieldHp: number = 0,
...
export const createFullBlastShield = (
	id: string,
	blastLevel: number,
	bsLevel: number,
) => {
	return new Ship(
		id,
		"bs",
		fullHpByBsLevel(bsLevel),
		0,
		blastShieldHp(blastLevel),
	);
};

export const createFullAreaShield = (
	id: string,
	areaLevel: number,
	bsLevel: number,
) => {
