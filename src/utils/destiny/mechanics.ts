...
export const solveSingleAttacker = (dmg: Cover, allDefenders: Ship[], remainingBlastCovers?: Cover[], remainingAreaCovers?: Cover[]): IterationOutput[][] => {
    ...
    return defenders.flatMap(d => {
        ...
        if(blastShields.length > 1 || areaShields.length > 1) {
            ...
            if(otherShips.length > 0) {
                return ret.remainingBlastCovers.flatMap(blasts => {
                    return ret.remainingAreaCovers.flatMap(areas => {
                        ...
                        return solveSingleAttacker(dmg, otherShips, blasts, areas).map(r => [ret2, ...r])
                    })
                })
            } else {
                return ret.remainingBlastCovers.flatMap(blasts => {
            ...
            if(otherShips.length > 0) {
                return solveSingleAttacker(dmg, otherShips, remBlastShields, remAreaShields).map(r => [ret2, ...r])
            } else {
                return [[ret2]]
            }
        }
...
const singleShieldIteration = (defender: Ship, dmg: number, remainingBlastCover: number, remainingAreaCover: number): SimpleIterationOutput => {
    //fully covered blast
...
const hitCoversWithDmg = (dmg: number, covers: Cover[]): Cover[][] => {
    return covers.flatMap(c => {
        ...
        if(dmg > c.amount) {
            ...
            return hitCoversWithDmg(dmg - c.amount, otherCovers).map(a => [zc, ...a])
        } else {
...
const multiShieldIteration = (defender: Ship, dmg: number, remainingBlastCovers: Cover[], remainingAreaCovers: Cover[]): MultiIterationOutput => {
    const fullBlastCover = remainingBlastCovers.reduce((acc, s) => s.amount + acc, 0)
    const fullAreaCover = remainingAreaCovers.reduce((acc, s) => s.amount + acc, 0)
