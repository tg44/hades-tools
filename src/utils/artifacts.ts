		...
		.map((m) => {
			...
			if (
				...
				return {
					...
					milestones: Array.isArray(mod.UnlockBlueprints)
						? mod.UnlockBlueprints
						: [mod.UnlockBlueprints ?? 1],
					tier: mod.AwardLevel,
					icon: mod.Icon,
...
export const basicCalc = (
	bps: {current: number; target: number},
	rate: number[],
	siblings: number,
) => {
	return {
...
interface TempModuleCalcData {
	bestCaseArtiNeedToTarget: number;
	worstCaseArtiNeedToTarget: number;
	avgCaseArtiNeedToTarget: number;
...
export const nonBasicCalc = (
	siblings: ModuleArtifactInfo[],
	bps: number[],
	selected: ModuleArtifactInfo,
	target: number,
	drops: number[],
	bonus: number,
) => {
	const data: TempModuleCalcData[] = siblings.map((s, idx) => {
		const allPossibleDrops = Array.from(
			{length: drops[1] - drops[0] + 1},
			(v, k) => (k + drops[0]) * (1 + bonus / 100),
		);
		const isSelected = s.name === selected.name;
		const currentBps = bps[idx] ?? 0;
		const currentTarget = isSelected
			? target
		...
		return {
			...
			bestCaseArtiNeedToTarget: Math.ceil(
				t / allPossibleDrops[allPossibleDrops.length - 1],
			),
			avgCaseArtiNeedToTarget: Math.ceil(
				t /
					(allPossibleDrops.reduce((a, b) => a + b, 0) /
						allPossibleDrops.length),
			),
		};
	});

	const rec = (
		data: TempModuleCalcData[],
		acc: (ArtiResult & {siblings: number})[],
	): ArtiResult => {
